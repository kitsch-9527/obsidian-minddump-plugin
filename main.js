  ...
  renderFullInput(container) {
    ...
    const refreshEmbedPreviews = () => {
      ...
      while ((m = re.exec(text)) != null) {
        const raw = m[1].trim();
        const vaultPath = (0, import_obsidian3.normalizePath)(raw);
        if (seen.has(vaultPath))
          continue;
        seen.add(vaultPath);
        const af = this.app.vault.getAbstractFileByPath(vaultPath);
        if (!(af instanceof import_obsidian3.TFile) || !isVaultImagePath(af.path))
          continue;
    ...
    const renderAttachmentTray = () => {
      ...
      selectedAttachments.forEach((attachment, index) => {
        ...
        if (attachment.type === "image") {
          const imgFile = this.app.vault.getAbstractFileByPath((0, import_obsidian3.normalizePath)(attachment.path));
    ...
    createToolBtn("\u{1F5BC}", t("attachmentPlaceholder", this.lang), () => openAttachmentPicker());
    addToolDivider();
