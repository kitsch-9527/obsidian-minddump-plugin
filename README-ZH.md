...
MindDump 是一个为 [Obsidian](https://obsidian.md) 设计的随手记插件，让你能够快速捕捉想法、记录灵感，并轻松管理碎片化笔记。支持标签嵌套、附件上传、日历视图、全文搜索等功能，完美融入 Obsidian 的工作流。

## 插件预览

### 主工作区

![Preview_workspace](demo/jot_workspace_cn.png)

### 侧边栏

![Preview_sidebar](demo/jot_sidebar_cn.png)

### MindDump笔记

![Preview_jot笔记](demo/jot_markdown_cn.png)

## 1 ✨ 特性

- **快速记录** – 一键打开记录窗口，快速写下想法
- **智能标签** – 支持标签嵌套（如 `工作/项目A`），输入时自动提示已有标签
- **来源与附件** – 可添加来源信息和附件（图片、文件），附件自动命名（`minddump-YYYYMMDD-序号`）
...
- **零干扰** – MindDump 仅在指定文件夹生成 .md 文件，不影响已有笔记。卸载后，记录依然完整可读。
- **轻量高效** – 增量加载，文件缓存，性能优化
...
5. 返回社区插件列表，启用 `MindDump` 插件。

### 2.2 手动安装
1. 从 [Releases](https://github.com/kitsch-9527/obsidian-minddump-plugin/releases)下载最新版本的 `main.js`、`manifest.json`、`styles.css`
2. 在你的 Obsidian 仓库中创建文件夹：`.obsidian/plugins/obsidian-minddump/`
3. 将下载的三个文件放入该文件夹
...
### 3.1 打开记录面板
- 点击左侧边栏的 ✨ 图标
- 或使用命令面板：`MindDump: 快速记录` / `MindDump: 打开随手记视图`
- 也可通过右键菜单将选中文本保存为 MindDump
...
| 选项 | 说明 |
|------|------|
| 保存文件夹 | 记录文件存放位置（默认 `MindDump`） |
| 附件存放目录 | 附件保存路径（默认 `MindDump/attachments`） |
| 记录模式 | `每天一个文件` 或 `单个文件` |
| 文件名格式 | 多文件模式下的文件名格式，如 `minddump-YYYYMMDD` |
...
### 7.1 多文件模式
- 每天一个文件，文件名按配置格式生成（如 `minddump-20260326.md`）
- 文件内每条记录格式：
...
### 7.2 单文件模式
- 所有记录保存在 `minddumps.md` 中，同样采用上述格式
...
- 本插件 fork 自 [obsidian-jot-plugin](https://github.com/ichris007/obsidian-jot-plugin)，原作者 [ichris007](https://github.com/ichris007)。在此基础上有大量增强和修改，感谢原作者的启发和基础工作。
